-- The board's projection now carries the notice's file, as a storage key. The
-- display route signs it before it reaches the browser, so the bucket stays
-- private and a link stops working once the message comes down.
